...
os.chdir("/Volumes/AC_Thunder_1/OFES_GAMMA_amean/final/downsampled/")

dataset = Dataset("OFES_GAMMA_amean_01_downsampled.nc", mode = 'r')
...
for tt in range(1,65):
    print ['\n ',tt,'\r']  
    
    for ii in range(0,len(lon)):
        print ' '.join(['Year',str(tt).zfill(2),'longitude index',str(ii).zfill(4)])
        os.chdir("/Volumes/AC_Thunder_1/OFES_GAMMA_amean/final/downsampled/")
        dataset = Dataset(''.join(['OFES_GAMMA_amean_',str(tt).zfill(2),'_downsampled.nc']), mode = 'r')
        rhol = dataset.variables['GAMMA'][:,0:yt,ii]
        dataset.close()
        drdzl = np.gradient(rhol,depth, axis=0)
        
        for jj in range(0,len(lat[0:yt])):
            ...
            g = 9.806 - .5*(9.832-9.780)*np.cos(2*lat[jj]*np.pi/180)
            N2 = -g/rho*drdz
        ...
        ncfile = ''.join(['Rossby_SL_OFES_',str(tt).zfill(2),'_',"%04.0d"%(ii,),'.nc'])        
